...
export const useChunkUpload = () => {
  ...
  const addFiles = useCallback(
    (files: File[], onComplete?: (fileId: string, success: boolean) => void) => {
      ...
      syncUploads((prev) => {
        ...
        files.forEach((file) => {
          ...
          console.log(
            `📄 [useChunkUpload] File: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`
          );
          ...
          if (useChunking) {
            ...
            uploader = new ChunkUploader(file, {
              ...
              onChunkError: (chunkIndex, error) => {
                console.error(
                  `❌ [useChunkUpload] Chunk ${chunkIndex} failed for ${file.name}:`,
                  error
                );
              },
            });
  ...
  const startUpload = useCallback(
    async (uploadId: string, onComplete?: (fileId: string, success: boolean) => void) => {
          ...
          const progressInterval = setInterval(() => {
            syncUploads((prev) => {
              ...
              if (current && current.status === 'uploading') {
                ...
                current.progress.uploadedBytes = Math.floor(
                  (current.progress.percentage / 100) * current.file.size
                );
                updated.set(uploadId, current);
              }
              return updated;
  ...
  const retryUpload = useCallback(
    (uploadId: string, onComplete?: (fileId: string, success: boolean) => void) => {
      const item = uploadsRef.current.get(uploadId);
      if (!item) {
